...
/* USBSID-Player vendor patch: USP_NO_THREADS.
 *
 * reSIDfp builds these tables on six threads. wasm has no std::thread without
 * -pthread, which needs SharedArrayBuffer and COOP/COEP headers this project
 * deliberately does not require, so the constructor throws and, with the caller
 * built -fno-exceptions, aborts the module outright.
 *
 * Under USP_NO_THREADS the same lambdas are called in order instead. Same tables,
 * built once at startup, a few milliseconds slower. Native builds keep threads.
 *
 * RE-APPLY THIS WHEN RE-VENDORING reSIDfp. See _project/PROGRESS.md.
 */
#if defined(USP_NO_THREADS)
    filterSummer();
    filterMixer();
    filterGain();
    filterResonance();
