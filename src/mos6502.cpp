...
bool mos6502::loginstructions = false;
...
void mos6502::RunN(uint32_t n, uint64_t& cycleCount)
{
	uint32_t c = 0;
	uint8_t opcode = 0;
	Instr instr;
	...
	for(;;)
	{
		...
		if (loginstructions) dump_regs_insn(opcode, instr.cycles);
...
unsigned short mos6502::d_address = 0;
...
void mos6502::dump_regs()
{
  std::stringstream sflags;
  if(IF_NEGATIVE())  sflags << "NF ";
  if(IF_OVERFLOW())  sflags << "OF ";
  if(IF_BREAK()) sflags << "BCF ";
  if(IF_DECIMAL()) sflags << "DMF ";
  if(IF_INTERRUPT()) sflags << "IDF ";
  if(IF_ZERO())  sflags << "ZF ";
  if(IF_CARRY())  sflags << "CF ";

  std::stringstream pflags;
  if(IF_NEGATIVE())  pflags << "1";
  else  pflags << "0";
  if(IF_OVERFLOW())  pflags << "1";
  else  pflags << "0";
  pflags << "-";
  if(IF_BREAK()) pflags << "1";
  else  pflags << "0";
  if(IF_DECIMAL()) pflags << "1";
  else  pflags << "0";
  if(IF_INTERRUPT()) pflags << "1";
  else  pflags << "0";
  if(IF_ZERO())  pflags << "1";
  else  pflags << "0";
  if(IF_CARRY())  pflags << "1";
  else  pflags << "0";

  printf("PC=%04x(%04x) M=%04X,A=%02x X=%02x Y=%02x SP=%02x(%04x) NV-BDIZC: %s flags= %s\n",
    GetPC(),Read(GetPC()),
    Read(d_address)<<8|Read(d_address),
		// 0x0,
    GetA(),GetX(),GetY(),
    GetS(),Read((0x0100+GetS())<<8)|Read(0x0100+GetS()),
    pflags.str().c_str(),
    sflags.str().c_str());
}

void mos6502::dump_regs_insn(uint8_t insn, uint8_t cycles)
{
  printf("INSN=%02X '%-9s' ADDR: $%04X VAL: $%02X CYC=%u ",
    insn,
    opcodenames[insn],
    d_address,
    Read(d_address),
