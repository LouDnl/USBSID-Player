 ...
 *
 */
void mos6526::glue_c64(mos6510 * _cpu)
{
  cpu = _cpu;
 ...
 * @return uint8_t
 */
uint8_t mos6526::read_register(uint8_t reg)
{
  uint8_t data = 0;
  switch(reg) {
 ...
 * @param value
 */
void mos6526::write_register(uint8_t reg, uint8_t value)
{
  w_shadow[reg] = value;
  switch(reg) {
    /* data port a (0x0), keyboard matrix cols and joystick #2 */
 ...
 */
void mos6526::write_prab_bits(uint8_t a, uint8_t b, bool state)
{
  uint8_t mask;
  if (state) { /* ON */
    /* Set bus */
 ...
 */
uint_least16_t mos6526::vic_base_address(void)
{
